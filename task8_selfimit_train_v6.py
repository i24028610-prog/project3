"""Task8 main training program.

用途：
- 加载 Task8 精英轨迹数据和 Task6 基础数据。
- 继续训练俄罗斯方块动作策略网络。
- 在训练和评估阶段使用 legal mask、安全重排和生存优先策略。
- 输出可供 task7_task3_sim.py 验证使用的 model_best_action.pt / model_last_action.pt。

当前策略重点：
控制消 2 行、消 3 行的比例，并用危险局面保命策略兜底，目标是总消行 11000 行结束且 2/3 行贡献占总消行 >= 15%。
当前目标口径：总消行达到 11000 行结束，2/3 行贡献占总消行 >= 15%。
"""
...
def combo_clear_ratio(single_clears: float, double_clears: float, triple_clears: float) -> float:
    total_lines = float(single_clears) + 2.0 * float(double_clears) + 3.0 * float(triple_clears)
    if total_lines <= 0.0:
        return 0.0
    combo_lines = 2.0 * float(double_clears) + 3.0 * float(triple_clears)
    return float(combo_lines) / total_lines
...
class ActionDatasetBase(Dataset):
    def __init__(self) -> None:
        ...
        self.lines_gain_immediate: np.ndarray
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            "lines_gain_immediate": torch.tensor(int(self.lines_gain_immediate[idx]), dtype=torch.long),
    ...
    def finalize_and_filter(self, seed: int, name: str) -> None:
        ...
        self.lines_gain_immediate = self.lines_gain_immediate[keep_idx]
...
class BaseTeacherDataset(ActionDatasetBase):
    def __init__(self, data_dir: str, seed: int) -> None:
        ...
        self.lines_gain_immediate = np.zeros(n, dtype=np.int64)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if pack is not None and "lines_gain_immediate" in pack:
            self.lines_gain_immediate = np.asarray(pack["lines_gain_immediate"]).astype(np.int16).clip(0, 4).astype(np.int64)
            self.is_clear_gain = (self.lines_gain_immediate > 0).astype(np.int64)
        else:
            self.lines_gain_immediate = self.is_healthy_clear_sample.copy().astype(np.int64)
        ...
        multi_clear_bonus = np.zeros(n, dtype=np.float32)
        multi_clear_bonus[self.lines_gain_immediate == 1] = 0.10
        multi_clear_bonus[self.lines_gain_immediate == 2] = 3.00
        multi_clear_bonus[self.lines_gain_immediate >= 3] = 8.00
        self.sample_weights = np.clip(
            self.sample_weights + multi_clear_bonus,
            sample_weight_clip_min,
            max(sample_weight_clip_max, 8.0),
        ).astype(np.float32)
...
def combo_setup_score_eval(board_occ: np.ndarray) -> float:
    fills = board_occ.sum(axis=1)
    features = eval_board_features(board_occ)
    score = 0.0
    score += 1.0 * float(np.sum(fills == BOARD_COLS - 1))
    score += 3.0 * float(np.sum(fills == BOARD_COLS - 2))
    score += 1.4 * float(np.sum(fills == BOARD_COLS - 3))
    score -= 0.35 * features["holes"]
    score -= 0.08 * max(0.0, features["max_h"] - 11.0)
    return float(score)


def combo_action_value_eval(lines: int, board_after: np.ndarray) -> float:
    if lines >= 3:
        return 120.0
    if lines == 2:
        return 24.0
    if lines == 1:
        return -2.0
    return combo_setup_score_eval(board_after)
...
def choose_action_greedy(
    ...
    policy_topk: int = DEFAULT_EVAL_POLICY_TOPK,
    safety_weight: float = DEFAULT_EVAL_SAFETY_WEIGHT,
    combo_weight: float = DEFAULT_EVAL_COMBO_WEIGHT,
    ...
    for action_idx, policy_value in zip(top_actions.tolist(), policy_values.tolist()):
        ...
        safe = is_survival_safe_transition(board_before, board_after)
        catastrophic = is_catastrophic_transition(board_before, board_after)
        candidate_rows.append(
            {
                "action": int(action_idx),
                "policy": float(policy_value),
                "safety": placement_safety_score(board_before, board_after, lines),
                "clear_value": combo_action_value_eval(lines, board_after),
                "lines": float(lines),
    ...
    if not candidate_rows:
        ...
        non_catastrophic = [row for row in candidate_rows if float(row["catastrophic"]) <= 0.5]
        if non_catastrophic:
            candidate_rows = non_catastrophic
        ...
        healthy = before_feat["max_h"] <= 8.0 and before_feat["holes"] <= 1.0 and before_feat["bump"] <= 16.0

        if danger:
            safe_rows = [row for row in candidate_rows if float(row["safe"]) > 0.5]
            if safe_rows:
        ...
        if healthy:
            clear_values = [float(row["clear_value"]) for row in candidate_rows]
            combo_scale = 1.25
        elif danger:
            clear_values = [clear_score(int(row["lines"])) if float(row["lines"]) >= 2.0 else 0.0 for row in candidate_rows]
            combo_scale = 0.20
        else:
            clear_values = [
                float(row["clear_value"]) if float(row["lines"]) >= 2.0 else 0.35 * float(row["clear_value"])
                for row in candidate_rows
            ]
            combo_scale = 0.65
        clear_arr = np.asarray(clear_values, dtype=np.float32)

        local_safety_weight = float(safety_weight) * (2.25 if danger else (0.80 if healthy else 1.25))
        local_combo_weight = float(combo_weight) * combo_scale
        line_bonus = np.asarray(
            [
                float(combo_weight) * (0.45 if danger else 1.25)
                if float(row["lines"]) >= 2.0
                else 0.0
                for row in candidate_rows
            ],
            dtype=np.float32,
        )
        combined = 0.45 * norm(policy_arr) + local_safety_weight * norm(safety_arr) + local_combo_weight * norm(clear_arr) + line_bonus
...
def rollout_one_episode_strict(
    ...
    return {
        ...
        "clear_score": float(game.clear_score_total),
        "single_clears": int(game.single_clears),
        "double_clears": int(game.double_clears),
        "triple_clears": int(game.triple_clears),
...
class EvalSummary:
    mean_lines: float
    mean_clear_score: float
    mean_single_clears: float
    mean_double_clears: float
    mean_triple_clears: float
    mean_combo_clear_ratio: float
    median_lines: float
    std_lines: float
    min_lines: float
    max_lines: float
...
def run_eval(
    ...
    clear_score_list: List[float] = []
    single_clear_list: List[float] = []
    double_clear_list: List[float] = []
    triple_clear_list: List[float] = []
    ...
    for i in range(episodes):
        ...
        clear_score_list.append(float(result["clear_score"]))
        single_clear_list.append(float(result["single_clears"]))
        double_clear_list.append(float(result["double_clears"]))
        triple_clear_list.append(float(result["triple_clears"]))
        ...
        if verbose:
            print(
                ...
                f"clear_score={float(result['clear_score']):.0f} "
                f"score={int(result['score']):06d} stop={result['stop_reason']}"
            )

    lines_arr = np.asarray(line_list, dtype=np.float32)
    clear_score_arr = np.asarray(clear_score_list, dtype=np.float32)
    single_arr = np.asarray(single_clear_list, dtype=np.float32)
    double_arr = np.asarray(double_clear_list, dtype=np.float32)
    triple_arr = np.asarray(triple_clear_list, dtype=np.float32)
    pieces_arr = np.asarray(piece_list, dtype=np.float32)

    mean_lines = float(lines_arr.mean()) if lines_arr.size > 0 else 0.0
    mean_clear_score = float(clear_score_arr.mean()) if clear_score_arr.size > 0 else 0.0
    mean_single_clears = float(single_arr.mean()) if single_arr.size > 0 else 0.0
    mean_double_clears = float(double_arr.mean()) if double_arr.size > 0 else 0.0
    mean_triple_clears = float(triple_arr.mean()) if triple_arr.size > 0 else 0.0
    mean_combo_clear_ratio = combo_clear_ratio(
        mean_single_clears,
        mean_double_clears,
        mean_triple_clears,
    )
    ...
    metric = (
        mean_clear_score
        + 900.0 * mean_combo_clear_ratio
        - 1800.0 * max(0.0, TARGET_COMBO_CLEAR_RATIO - mean_combo_clear_ratio)
    ...
    return EvalSummary(
        ...
        mean_clear_score=mean_clear_score,
        mean_single_clears=mean_single_clears,
        mean_double_clears=mean_double_clears,
        mean_triple_clears=mean_triple_clears,
        mean_combo_clear_ratio=mean_combo_clear_ratio,
...
def run_one_epoch(
    ...
    for batch in loader:
        ...
        lines_gain_immediate = batch["lines_gain_immediate"].to(device)
        ...
        lines_gain_v = lines_gain_immediate[valid_mask]
        ...
        with torch.set_grad_enabled(is_train):
            ...
            dyn_weight = dyn_weight + 0.10 * (lines_gain_v == 1).float()
            dyn_weight = dyn_weight + 3.00 * (lines_gain_v == 2).float()
            dyn_weight = dyn_weight + 8.00 * (lines_gain_v >= 3).float()

            dyn_weight = torch.clamp(
                dyn_weight,
                min=float(sample_weight_clip_min),
                max=float(sample_weight_clip_max + 0.50),
            )
    ...
    with path.open("w", encoding="utf-8-sig", newline="") as f:
        writer = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "heldout_mean_lines",
                "heldout_mean_clear_score",
                "heldout_mean_single_clears",
                "heldout_mean_double_clears",
                "heldout_mean_triple_clears",
                ...
                "heldout_metric",
                "shadow_mean_lines",
                "shadow_mean_clear_score",
                "shadow_mean_single_clears",
                "shadow_mean_double_clears",
                "shadow_mean_triple_clears",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--elite_data_dir",
        type=str,
        default=str(PROJECT_ROOT / "out_task8_elite_v6" / "v6_struct_mainline"),
    )
    parser.add_argument(
        "--base_data_dir",
        type=str,
        default=str(PROJECT_ROOT / "out_task6_data"),
    )
    parser.add_argument(
        "--init_model",
        type=str,
        default=str(PROJECT_ROOT / "out_task8_selfimit_v6_mainline_next" / "model_best_action.pt"),
    )
    parser.add_argument(
        "--out_dir",
        type=str,
    ...
    parser.add_argument("--sample_weight_clip_max", type=float, default=8.00)
    parser.add_argument("--eval_policy_topk", type=int, default=DEFAULT_EVAL_POLICY_TOPK)
    parser.add_argument("--eval_safety_weight", type=float, default=DEFAULT_EVAL_SAFETY_WEIGHT)
    parser.add_argument("--eval_combo_weight", type=float, default=DEFAULT_EVAL_COMBO_WEIGHT)
...
def main() -> None:
    ...
    print(
        ...
        f"eval_combo_weight={args.eval_combo_weight:.3f} "
        f"target_combo={TARGET_COMBO_CLEAR_RATIO * 100:.1f}%"
    )
    ...
    info_payload = {
        ...
        "eval_combo_weight": args.eval_combo_weight,
        "target_combo_clear_ratio": TARGET_COMBO_CLEAR_RATIO,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        heldout_eval = run_eval(
            ...
            combo_weight=args.eval_combo_weight,
        ...
        row = {
            ...
            "heldout_mean_clear_score": heldout_eval.mean_clear_score,
            "heldout_mean_single_clears": heldout_eval.mean_single_clears,
            "heldout_mean_double_clears": heldout_eval.mean_double_clears,
            "heldout_mean_triple_clears": heldout_eval.mean_triple_clears,
            "heldout_mean_combo_clear_ratio": heldout_eval.mean_combo_clear_ratio,
            ...
            "shadow_mean_clear_score": shadow_eval.mean_clear_score,
            "shadow_mean_single_clears": shadow_eval.mean_single_clears,
            "shadow_mean_double_clears": shadow_eval.mean_double_clears,
            "shadow_mean_triple_clears": shadow_eval.mean_triple_clears,
            "shadow_mean_combo_clear_ratio": shadow_eval.mean_combo_clear_ratio,
        ...
        print(
            ...
            f"heldout clear_score={heldout_eval.mean_clear_score:.2f} "
            f"1/2/3={heldout_eval.mean_single_clears:.2f}/{heldout_eval.mean_double_clears:.2f}/{heldout_eval.mean_triple_clears:.2f} "
            f"combo={heldout_eval.mean_combo_clear_ratio * 100:.2f}% "
            f"lines={heldout_eval.mean_lines:.4f} median={heldout_eval.median_lines:.4f} "
            ...
            f"shadow clear_score={shadow_eval.mean_clear_score:.2f} "
            f"1/2/3={shadow_eval.mean_single_clears:.2f}/{shadow_eval.mean_double_clears:.2f}/{shadow_eval.mean_triple_clears:.2f} "
            f"combo={shadow_eval.mean_combo_clear_ratio * 100:.2f}% "
            f"lines={shadow_eval.mean_lines:.4f} median={shadow_eval.median_lines:.4f} "
            ...
            torch.save(
                {
                    ...
                    "heldout_eval": {
                        ...
                        "mean_clear_score": heldout_eval.mean_clear_score,
                        "mean_single_clears": heldout_eval.mean_single_clears,
                        "mean_double_clears": heldout_eval.mean_double_clears,
                        "mean_triple_clears": heldout_eval.mean_triple_clears,
                        "mean_combo_clear_ratio": heldout_eval.mean_combo_clear_ratio,
                    ...
                    "shadow_eval": {
                        ...
                        "mean_clear_score": shadow_eval.mean_clear_score,
                        "mean_single_clears": shadow_eval.mean_single_clears,
                        "mean_double_clears": shadow_eval.mean_double_clears,
                        "mean_triple_clears": shadow_eval.mean_triple_clears,
                        "mean_combo_clear_ratio": shadow_eval.mean_combo_clear_ratio,
    ...
    print(
        ...
        f"heldout metric={final_heldout_eval.metric:.4f} clear_score={final_heldout_eval.mean_clear_score:.2f} "
        f"1/2/3={final_heldout_eval.mean_single_clears:.2f}/{final_heldout_eval.mean_double_clears:.2f}/{final_heldout_eval.mean_triple_clears:.2f} "
        f"combo={final_heldout_eval.mean_combo_clear_ratio * 100:.2f}% "
        f"mean={final_heldout_eval.mean_lines:.4f} "
        ...
        f"shadow metric={final_shadow_eval.metric:.4f} clear_score={final_shadow_eval.mean_clear_score:.2f} "
        f"1/2/3={final_shadow_eval.mean_single_clears:.2f}/{final_shadow_eval.mean_double_clears:.2f}/{final_shadow_eval.mean_triple_clears:.2f} "
        f"combo={final_shadow_eval.mean_combo_clear_ratio * 100:.2f}% "
        f"mean={final_shadow_eval.mean_lines:.4f} "
